...
namespace _0406_Server
{
    internal class Program
    {
        ...
        public void Server_RecvData(Socket sock, string msg)
        {
            ...
            else if (sp1[0].Equals(Packet.WITHDRAW_ACCOUNT))
            {
                string[] sp2 = sp1[1].Split('#');
                Control.Instance.WithdrawAccount(sock,
                    int.Parse(sp2[0]), int.Parse(sp2[1]));
            }
            else if (sp1[0].Equals(Packet.INQUIRE_ACCOUNT))
            {
                string[] sp2 = sp1[1].Split('#');
                Control.Instance.InquireAccount(sock,
                    int.Parse(sp2[0]));
            }
            else if (sp1[0].Equals(Packet.DELETE_ACCOUNT))
            {
                string[] sp2 = sp1[1].Split('#');
                Control.Instance.DeleteAccount(sock,
                    int.Parse(sp2[0]));
            }
            else if (sp1[0].Equals(Packet.PRINT_ALL ))
            {
                string[] sp2 = sp1[1].Split('#');
                Control.Instance.PrintAll(sock);
            }
        }
